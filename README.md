...
- Only Android, iOS and JVM publications are produced. The macOS and Windows binaries bundled in
  the JVM artifacts are **unsigned** (the detached code signatures under `external/codesign` can
  only be regenerated by the upstream maintainer), so they are not usable on macOS and are not
  meant for distribution. Use the upstream coordinates for desktop.
