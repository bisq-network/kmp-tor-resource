...
- Signing key: `Bisq kmp-tor-resource signing <kmp-tor-resource@bisq.network>`, fingerprint
  ...
  ```
- Only Android, iOS and JVM publications are produced. The detached Developer ID signatures under
  `external/codesign` can only be regenerated by the upstream maintainer, so this fork gives the
  Apple compilations (macOS, iOS Simulator) a deterministic **ad-hoc** signature at packaging
  (`--adhoc-codesign`, via `rcodesign`) and leaves the Windows binaries unsigned. Ad-hoc signed
  code loads on Apple Silicon and in the iOS Simulator, and runs locally on macOS, but is not
  notarized: do not distribute the desktop binaries, use the upstream coordinates for that.
