...
This is [bisq-network/kmp-tor-resource](https://github.com/bisq-network/kmp-tor-resource), a fork of
[05nelsonm/kmp-tor-resource](https://github.com/05nelsonm/kmp-tor-resource) that publishes tor
...
- Signing key: `Bisq kmp-tor-resource signing <kmp-tor-resource@bisq.network>`, fingerprint
  ...
  `keys.openpgp.org`:
  ```shell
  gpg --import kmp-tor-resource-signing-key.asc
  # or: gpg --keyserver hkps://keys.openpgp.org --recv-keys 4ECEE529847B49F181D69A7C9BC47C3D2AE1FF8B
  gpg --verify resource-exec-tor-gpl-409.12.0.pom.asc resource-exec-tor-gpl-409.12.0.pom
  ```
- Only Android, iOS and JVM publications are produced. The macOS and Windows binaries bundled in
  the JVM artifacts and the iOS Simulator slice of `LibTor.xcframework` are **unsigned** (the
  detached code signatures under `external/codesign` can only be regenerated by the upstream
  maintainer). The JVM desktop binaries are not usable on macOS and not meant for distribution;
  use the upstream coordinates for desktop. The simulator slice works inside an app because Xcode
  signs embedded frameworks at build time, but cannot be loaded by a bare test binary.
...
dependencies {
    implementation("network.bisq.kmp-tor:resource-exec-tor-gpl:409.12.0")   // Android
    implementation("network.bisq.kmp-tor:resource-noexec-tor-gpl:409.12.0") // iOS
}
```
